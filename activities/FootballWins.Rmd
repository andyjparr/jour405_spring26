...
It's tough to say. While things like the p-value and adjusted r-squared seem to say that there's a statistically significant pattern in play here, the minimum and maximum values show that it can still be off by four in either direction. That might not feel like much, but for an NFL season that's 25% of each team's games. If the model has an eight-win team that could really be as high as 12 or as low as four.
...
```{r}
selected_vars <- teams |>  select(`Yards Gained`, Takeaways, `Points Scored`, Wins, Giveaways, 'Yards Allowed')
cor(selected_vars)
```
...
Points scored has the greatest positive impact on wins, while yards allowed has the highest negative impact on wins. All of these variables seem to have some kind of effect on team wins, but yards gained has by far the least impact of the variables available in this model.
...
Analyze the team's yards allowed compared to other NFL teams for the week, pointing out the pattern to help explain the team's success (or lack there of). You could also use the other major factor, points scored, to compare it around the league. Basically the best way to contextualize this data without saying "multiple regression model" is throw it into the greater overall league picture. Show how the team you're covering performed in these predictive metrics compared to the other teams who played.
